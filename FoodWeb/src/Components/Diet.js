...
const Diet = () => {
  ...
  const handleCategoryClick = async (category) => {
    try {
      const response = await fetch(`https://api.spoonacular.com/recipes/complexSearch?apiKey=${apiKey}&diet=${dietType}&number=10&addRecipeInformation=true&type=${category.toLowerCase()}`);
      if (!response.ok) {
