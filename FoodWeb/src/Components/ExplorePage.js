...
const categoryImages = [
  ...
  { name: 'Bread', image: 'https://i.pinimg.com/736x/52/f4/b3/52f4b300d1249c63a72ee0f97038ef99.jpg'},
...
const ExplorePage = () => {
  ...
  const apiKey = '9e1d7b28982140dd98e0a541530eb4a4';
  ...
  const handleCategoryClick = async (category) => {
    try {
      const response = await fetch(`https://api.spoonacular.com/recipes/complexSearch?apiKey=${apiKey}&number=10&addRecipeInformation=true&type=${category.toLowerCase().replace(/\s+/g, '-')}`);
      if (!response.ok) {
  ...
  const handleSearch = async (value) => {
    setQuery(value);
    try {
      const response = await fetch(`https://api.spoonacular.com/recipes/complexSearch?query=${value}&apiKey=${apiKey}&addRecipeInformation=true&number=10`);
      if (!response.ok) {
